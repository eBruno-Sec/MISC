...
## RUN 2 SUMMARY - the whole sweep in one table

| ticket | verdict | the one line that decides it |
|---|---|---|
| Q-001 | CLOSED + decision | engine shipped and dispatched once; `validated_on=["sessionlife"]` names an untracked lab |
| Q-002 | SPLIT | CSWSH closed; WS-frame injection never built; no technique record for CSWSH |
| Q-003 | OPEN | 0 hits for `postMessage` repo-wide; `WSTG-CLNT-11 -> none` |
| Q-004 | OPEN | only `graphql_batching_enabled` touches CWE-770; no rate-limit engine |
| Q-005 | OPEN | every CWE-1321 site is client-side; no body-borne `__proto__` probe |
| Q-006 | DELETE | the refusal is live code in `wstg_catalog.EXCLUDED` and counted in `tally['excluded']` |
| Q-007 | OPEN, smallest fix | 6 of 6 `conflicts()` rows are generated by an engine that does not exist |
| Q-009 | DELETE | a container of six sub-claims, all re-homed, two already closed |
| Q-015 | CLOSED | shares `_confirmed` with `risk_score`; oracle re-run 0/0 and 25/25 |
| Q-016 | CLOSED | `_read_controls` records the exception; the test distinguishes boom from empty |
| Q-017 | SPLIT | logs half closed; 13 raw vs 7 gated `get_findings` call sites remain |
| Q-018 | OPEN (LOW) | `_eng = None` on load failure and the guard is then skipped; reproduced live |
| Q-019 | CLOSE + fix oracle | 3 of 3 root causes fixed; oracle (c) would record a FAIL against a working fix |
| Q-021B | CLOSE | shipped and wired at `tools.py:3994`; 47 tests green; oracle 1's confidence clause is wrong |
| Q-021D | OPEN | `ingest()` works, `production()` stays 0, `advance()` has no caller outside tests |
| Q-021E | OPEN, re-scope down | Q-021B already emits `has_versions`; only the consumer is missing |
| Q-021F | OPEN (LOW) | technology reaches a report only via the delta section; `TECH_PROOF_LADDER` already exists |
| Q-022 | OPEN, narrowed | 662 unbacked claims removed, but the 3 findings that DO carry a control now read "NOT ESTABLISHED" |
| Q-023 | REWRITE | 2 sub-defects closed, consumer contract met, 1 live clause (`run_zap:<host>` dedup) |
| Q-030 | RE-SCOPE | built except D14 and D15; QUEUE.md and LEDGERS.md contradict each other |
| Q-035 | OPEN, unstarted | an experiment, not a defect; no measurement exists |
| Q-036 | REWRITE | at least 4 of the 15 defects are already fixed; one proposed fix is refuted in place |
| B-011+ | DELETE | a roadmap row in `BENCHMARK_MATRIX.md`, not a ticket; C/C++ UNSUPPORTED confirmed |
| `00438` | REWRITE | probed and CONFIRMED in `ebd96f45`; "unprobed" is an inference the seal cannot support |
| sublinear cost | DELETE | designed property; the "76 hours" projection is disproved |

## PROPOSED `docs/QUEUE.md` EDITS - this lane does not touch QUEUE.md. The Coordinator applies these.

**DELETE OUTRIGHT (5).** Each is measured, not argued:

1. **Q-006** - a standing decision already enforced in `wstg_catalog.EXCLUDED["WSTG-INPV-15"]` and
   counted in `tally['excluded']=5`. Tier 1/2 are a new ticket with a different premise if ever wanted.
2. **Q-009** - a container. All six sub-claims are re-homed and two are closed.
3. **Q-019's Rank 3b duplicate** (`QUEUE.md:1665`) - the same ticket as the Rank 0 entry at `:1346`.
4. **B-011+** - a roadmap row that already lives in `BENCHMARK_MATRIX.md`. Keep the one index row.
5. **the "unexplained sublinear per-URL cost" item** - explained by two measured causes; fold into
   the Q-019 close.

**REWRITE, do not work as filed (5).** Working these as written wastes a lane:

- **Q-023** down to its one live clause (`run_zap:<host>` key + `fresh()` dedup). Delete sub-defects
  1 and 3, the three-gate explanation and the "fifth unidentified cause".
- **Q-036** to "re-verify the 15 against current code, fold the survivors". D3's proposed fix is
  refuted in place at `planner.py:140`.
- **the `00438` item** to the honest statement: probed and confirmed in `ebd96f45`, selected by the
  model at cap 700, no claim in wp3, probe status unknowable from the seal.
- **Q-021E** re-scoped down - its producer half is discharged by Q-021B.
- **Q-030** re-marked "built except D14 and D15"; promote those two into `QUEUE.md`.

**CLOSE (3).** Q-015 and Q-016 are already listed as closed in the ranked table at `QUEUE.md:1025`
while their bodies still read `proposed` - resolve the duplication in both directions:

- **Q-015**, **Q-016** - stamp the closing commit on the bodies.
- **Q-021B** - shipped, wired and tested. Record the two residuals as one small follow-up.

**FIX THREE MIS-SPECIFIED ORACLES while editing.** All three would record a FAIL against working
code, and a lane that trusts them will revert a good fix:

- **Q-019 oracle (c)** - "distinct URLs `http_probe` touched > 200". Rewrite to "distinct URLs
  reaching an INJECTION engine > 200" (passes at 432).
- **Q-022 oracle 2** - "the string `does NOT reproduce` must not appear". Rewrite to "the heading is
  not `How this was confirmed` and the text opens with `NO NEGATIVE CONTROL WAS RECORDED`".
- **Q-021B oracle 1** - "`confidence in CVE_ELIGIBLE`". Header-derived facts are LOW by design, which
  is what the ticket's own FP-risk section demands. Require `version` non-empty and
  `proof_state == "version_suspected"`; move the CVE-eligibility assertion to the served-artifact path.

**THREE STATE CONTRADICTIONS INSIDE `docs/QUEUE.md` ITSELF.** Each is one ticket in two states:

- `:1050` "Q-023, Q-013 and Q-014 are all closed" vs `:2810` Q-023 `**HIGH** proposed`.
- `:1025` Q-015/Q-016/Q-017 struck through as CLOSED vs their bodies at `:1882/:1905/:1923` `proposed`.
- `:1042` "Q-030 is designed, not built" vs `docs/LEDGERS.md:2106` "Q-030 is complete as wiring".

**ONE PRODUCT-SOURCE EDIT for whoever owns `agent.py`** (this lane is read-only and did not make it):
delete the run-specific claim at `agent/agent.py:219` ("00438, the ninth case, is still unprobed").
It is measurably wrong in the word that matters, and a source comment asserting the state of one
benchmark run rots on the next run.

**ONE PROCESS ITEM.** Q-030/Q-035/Q-036 have no body in `docs/QUEUE.md`; they live in
`docs/QUEUE_ARCH.md`. Either inline them or add an explicit pointer, because the queue currently
names three tickets a reader cannot open.
