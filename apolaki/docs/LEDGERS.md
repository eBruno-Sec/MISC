...
do and is the reason this cycle is worth more than the fixes in it.

**Q-043 was already implemented.** The ticket says `Retry-After` appears nowhere in `agent/`. True
when filed, false now: `c02208d` landed a real per-origin policy, **and `QUEUE.md:785` already listed
Q-043 as CLOSED against it** while the ticket body sat marked `ready` a thousand lines further down.
The queue contradicted itself and I repeated the wrong half in a lane brief.

Measured against a loopback server that really answers 429/503, driven through the production
transport: delta-seconds honoured (2.006s), HTTP-date honoured (1.295s, correct at 1s date
resolution), 503 honoured (2.004s), and three negative controls flat at ~0.003s — absent header,
malformed value, and a 200 carrying the header. **The positive rows are what make those zeros mean
"no wait" rather than "no instrument".**

The real defect was one the ticket never named: **the cap bounds the HEADER, not the WAIT.** 270
seconds of parking under a 30-second cap. A hostile or misconfigured target could still stall a
mission, which is the whole reason a cap exists.

**Q-072: my own false claim, pinned in product source for days.** `agent/agent.py:219` read "00438,
the ninth case, is still unprobed", and every budget discussion since was priced against it. Measured
on mission `ebd96f45`: probed by **eight** engines and **confirmed** `sqli`, exactly like the other
eight cases; negative control `90cee81c` shows 0 rows and 0 findings. The selection model picks it at
index 58 with the cap at 700 — **12 slots spare** — so raising the cap could never have fixed a case
the cap already covered. The claim came from the wp3 seal having 0 mentions of it, but that seal
carries **claims only, no probed-cases list**. **Absence of a claim is not evidence of absence of a
probe**, and that is the same error as reading a zero off an apparatus that was never looking.

**Q-071: a shipped fix that inverts itself in production.** `control_status` scans top-level keys;
the only producer of negative controls (BIE) writes them nested. Coordinator-verified against the
live DB: **1057 findings, 3 carry a real control, `control_status` reports RECORDED for 0.** The
report prints "NO NEGATIVE CONTROL WAS RECORDED" beside a table listing three real controls for the
same finding. Its test is green because the fixture invents a top-level shape **no producer emits** —
the invented-fixture failure for the fourth time. And the ticket's "34 carry a control" does not
reproduce: it is 3, so the problem was worse than filed and the fix was measured against a population
that does not exist.

**Five tickets were DELETED as not being tickets at all** — a decision already enforced in code
(Q-006), a duplicate of Q-019 carried in two states, a roadmap row, an audit item two-thirds fixed,
and an observation that is interesting but is not a unit of work. **Three oracles would each record a
FAIL against working code**, one of them contradicting its own false-positive section.

The lesson the cycle keeps repeating: **a ticket is a hypothesis about the code, and the first
obligation of a lane is to test it.** Every lane this cycle that measured first found the ticket
wrong in scope, in mechanism, or in existence.

## ★★★ THE PLANNER'S ENTIRE NEGATIVE-EFFECTS MODEL IS GENERATED BY A PHANTOM. 2026-08-17 night

The tail sweep was sent to verify markers, not to find anything. It found this.

`engine_descriptor.conflicts()` returns **6 rows, and all six are produced by
`weak_password_reset`** - an engine with **no executor on any of the three surfaces**. Positive
control, same probe, same run: it returns six real `sqli` engine names, so the apparatus was
looking. It is the only one of 13 `EFFECTS` entries with a non-empty `invalidates`, which means
**the whole Sussman half of the planner model rests on an engine that cannot run.**

The sharpest row: the phantom is declared to invalidate the precondition of `session_lifecycle` -
the engine that actually destroys sessions - and `session_lifecycle` itself declares no effects at
all. So the model records the fictional engine clobbering the real one, and nothing records what the
real one does.

It also corrects the standing 08-10 recommendation, which said to "set `solver_only=True` (the field
exists)". **The field exists on 0 of 88 records.** A recommendation written from a declaration rather
than a fact, which is the defect this project keeps finding in its own code, this time in its own
advice. The work is now unblocked because Q-001 shipped, and it is three edits with no new engine.

Two more from the same sweep, both DELETIONS rather than work:

- **Q-006 is not a ready ticket, it is a standing decision already enforced in code.**
  `wstg_catalog.EXCLUDED["WSTG-INPV-15"]` refuses request smuggling under the no-collateral rule, and
  `coverage()` counts it in `tally[excluded]=5` rather than losing it in `none` - the honest shape.
  Tiers 1 and 2 were never begun and the ticket should say so or go.
- **Q-019 is fixed, and its own acceptance oracle would fail it.** Recorded because that is the more
  useful half: a ticket can be closed by the code while its stated success criterion still reads red,
  and only checking both catches it.

**Q-040 turned out to be two halves, and the ticket named the closed one.** The sqli side is fixed;
the open side is `nosqli`. The gate that shipped closes the single-sample case, and 18 of 120 ordered
triples still confirm on responses containing no injection - because ONE repeat cannot establish
stability on a BIMODAL page. Filed as Q-070 with a strict xfail carrying the measurement, rather than
rushed at the end of a cycle, because every candidate fix carries a false-negative risk pointing the
other way and this project has already paid once for a fix that traded one error class for the other.
