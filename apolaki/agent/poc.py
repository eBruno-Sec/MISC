"""Proof-of-concept rendering + evidence redaction.
...
submissions. Pure functions over plain dicts: no DB, no network. Ported from
OLYMPUS core/poc.py.
"""
...
def redact_headers(headers: dict) -> dict:
    """Mask credential-bearing header values while keeping header names, so a PoC
    still shows that auth is required without leaking the session."""
    out = {}
...
def finding_markdown(finding: dict, exchanges: list = None, redact: bool = True) -> str:
    ...
    if not urls and finding.get("target"):
        urls = [finding["target"]]
    if urls:
...
def mission_markdown(target: str, findings: list, exchanges_by_finding: dict,
    ...
    for f in findings:
        fid = f.get("id")
        body.append(finding_markdown(f, exchanges_by_finding.get(fid, []), redact))
        body.append("\n---\n")
