...
def test_the_shipped_conflict_set_is_empty_and_that_is_the_measured_answer():
    """Q-007. This test used to assert `conflicts()` is NON-empty and contains a `weak_password_reset`
    row. All six rows it ever returned were produced by that one entry, and MEASURED it has no executor
    on any surface -- no name matching reset/password/passwd in TOOL_PERMISSIONS (111) or the
    `_run*/_confirm*` methods (96) or CLAUDE_TOOLS (76), against six `sqli` names on the same probe.
    The entire Sussman half of the shipped model was generated by an engine that does not exist.

    It is not re-homed onto `session_lifecycle`, the engine that really does end a session: that engine
    mints a sacrificial account, never merges `session_headers` (`tools._sl_req`), and refuses to act on
    any credential value-shared with a live session (`tools._session_kill_is_safe`), so the engagement's
    `authenticated` observation survives it.

    So the honest shipped answer is EMPTY, and the reason is the non-destructive doctrine: every action
    that would delete an engagement-level observation is one Apolaki refuses to take. The machinery is
    proven over synthetic descriptors below and in test_effect_search.py -- that is where a mechanism
    with no shipped instance belongs."""
    assert ed.conflicts() == [], ed.conflicts()
    assert [t for t, e in ed.EFFECTS.items() if e.get("invalidates")] == []
    # NON-VACUITY: the apparatus is looking. The positive half of the same walk is large and real.
    assert len(ed.chains()) > 40, "conflicts() is empty because the walk found nothing at all"


def test_the_sussman_machinery_still_works_on_a_declared_negative_effect():
    """Negative control for the test above: `conflicts()` returns [] because the TABLE has no negative
    effect, not because the function stopped working. Hand it one and a row must appear."""
    d = ed.build()
    d["fake_rotator"] = dict(d["sqli_auth_bypass"], id="fake_rotator",
                             establishes=[], invalidates=["authenticated"])
    cf = ed.conflicts(d)
    assert cf, "conflicts() cannot see a declared negative effect"
    assert {t for t, o, c in cf} == {"fake_rotator"}, cf
...
def test_audit_endpoint_actually_serves_the_effects_layer():
    ...
    # Q-007: the conflict half is now EMPTY on the shipped tree, and the endpoint must report the empty
    # set rather than omit the key -- an operator reading a dashboard with no conflict field cannot tell
    # "no negative effects exist" from "nobody computed them". The count must still MATCH the list, which
    # is the assertion that actually catches a broken payload.
