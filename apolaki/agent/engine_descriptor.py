...
EFFECTS = {
    ...
    # onto `session_lifecycle`: see the note at that technique's PRECONDITIONS entry — the session that
    # engine ends is a sacrificial one it minted itself, value-checked disjoint from every live session
    # by `tools._session_kill_is_safe`, so the engagement's `authenticated` observation survives it.
    # `conflicts()` is therefore EMPTY, and that is the honest answer: every action that would delete an
    # engagement-level observation is one the non-destructive doctrine forbids. The negative half of the
    # model is machinery without a shipped instance, and it is tested as such.
