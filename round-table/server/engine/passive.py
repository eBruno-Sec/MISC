...
def run_passive(target: str, run_dir: Path, log) -> dict:
    data: dict = {"target": target, "domain": target}

    log("WHOIS via RDAP", phase="passive")
    data["whois"] = P.rdap_whois(target)
    w = data["whois"]
    if w:
    ...
    data["a_records"] = P.dns_query(target, "A")
    data["aaaa_records"] = P.dns_query(target, "AAAA")
    data["mx_records"] = P.dns_query(target, "MX")
    data["ns_records"] = P.dns_query(target, "NS")
    data["txt_records"] = P.dns_query(target, "TXT")
    data["caa_records"] = P.dns_query(target, "CAA")
    ...
    data["email"] = P.check_email_security(target, data["txt_records"])
    em = data["email"]
    ...
    data["http"] = P.check_headers(target)
    if data["http"].get("ok"):
        h = data["http"]["headers"]
    ...
    data["ssl"] = P.ssl_cert_info(target)
    ...
    subs, wildcards = P.crtsh_subdomains(target)
    data["subdomains"] = subs
    data["wildcards"] = wildcards
    data["sub_cats"] = {}
    for s in subs:
        cat, sev = P.classify_sub(s, target)
