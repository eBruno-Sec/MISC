"""
Remediation Playbook catalog (Spec 5).

For each vulnerability class, this returns a developer-facing fix: a short
summary plus copy-paste, language-specific secure-code snippets. Round Table
never touches the target's repo — these are snippets the pentester pastes into
the report so the dev can fix it themselves.
"""
from typing import Any, Optional

# Map a finding to a remediation entry by its `key` prefix and/or `tags`.
CATALOG: dict[str, dict[str, Any]] = {
    ...
    "idor": {
        "summary": "Enforce object-level authorization on every request: check the current user owns/at may access the referenced object server-side.",
        "fixes": [
...
# Resolve a finding to a catalog key. Order matters (first match wins).
_KEY_HINTS = [
...
def remediation_for(finding: dict) -> Optional[dict]:
    ...
    # LLM findings carry their own remediation inline.
    return None
